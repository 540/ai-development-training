...
function describe(risk: FunctionRisk): string {
  ...
  return `  ${risk.file}:${risk.line} ${risk.name} — complejidad ${risk.complexity}, cobertura ${coverage}, CRAP ${risk.crap.toFixed(2)}`
}
...
export function crapGate(report: CoverageReport | null): GateResult {
  ...
  if (failures.length === 0) {
    return { exitCode: 0, message: `Ninguna función supera el CRAP ${CRAP_THRESHOLD}.` }
  }
  ...
  return {
    exitCode: 1,
    message: `Estas funciones superan el CRAP ${CRAP_THRESHOLD}:\n${lines}`,
  }
}
